...
function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'http://localhost:8000/user/thirdlogin';
  options.tokenURL = options.tokenURL || 'http://localhost:8000/mapi/oauth/access_token';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.customHeaders = options.customHeaders || {};

  if (!options.customHeaders['User-Agent']) {
    options.customHeaders['User-Agent'] = options.userAgent || 'passport-plogin';
  }

  OAuth2Strategy.call(this, options, verify);
  this.name = 'plogin';
  this._userProfileURL = options.userProfileURL || 'http://localhost:8000/mapi/oauth/user';
